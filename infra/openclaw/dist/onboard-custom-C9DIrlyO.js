...
import { Bt as DEFAULT_PROVIDER, F as modelKey, j as buildModelAliasIndex } from "./auth-profiles-Cn5oo5Dj.js";
import { n as fetchWithTimeout } from "./fetch-timeout-DUo3Ogxz.js";
import { s as normalizeAlias } from "./shared-BdLUX_VG.js";
import { r as applyPrimaryModel } from "./model-picker-DlWdaDWE.js";
...
async function requestOpenAiVerification(params) {
	...
	const endpointUrl = new URL("chat/completions", resolvedUrl.endsWith("/") ? resolvedUrl : `${resolvedUrl}/`);
...
async function requestAnthropicVerification(params) {
	const resolvedUrl = isAzureUrl(params.baseUrl) ? transformAzureUrl(params.baseUrl, params.modelId) : params.baseUrl;
	const endpointUrl = new URL("messages", resolvedUrl.endsWith("/") ? resolvedUrl : `${resolvedUrl}/`);
	if (isAzureUrl(params.baseUrl)) endpointUrl.searchParams.set("api-version", "2024-10-21");
	const endpoint = endpointUrl.href;
	try {
		const res = await fetchWithTimeout(endpoint, {
