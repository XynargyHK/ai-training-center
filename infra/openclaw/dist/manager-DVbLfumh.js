...
import { P as resolveUserPath, R as sleep, S as clampNumber$1, V as truncateUtf16Safe, x as clampInt } from "./registry-DykAc8X1.js";
import { n as resolveAgentConfig, r as resolveAgentDir, s as resolveAgentWorkspaceDir } from "./agent-scope-BXfmtC-7.js";
import { t as createSubsystemLogger } from "./subsystem-BxhC68lz.js";
import { C as requireApiKey, w as resolveApiKeyForProvider } from "./model-selection-CAfIlaZ9.js";
import { t as isTruthyEnvValue } from "./env-DUOlMsCb.js";
import { n as formatErrorMessage } from "./errors-C7sgRdrp.js";
import { o as resolveSessionTranscriptsDirForAgent } from "./paths-BHRTOOlS.js";
import { n as onSessionTranscriptUpdate } from "./transcript-events-DUQC5hbS.js";
import { n as collectProviderApiKeysForExecution, r as executeWithApiKeyRotation, t as parseGeminiAuth } from "./gemini-auth-BPzKHwI5.js";
import { a as buildFileEntry, c as ensureDir, d as listMemoryFiles, f as normalizeExtraMemoryPaths, h as runWithConcurrency, i as sessionPathForFile, l as hashText, m as remapChunkLines, n as buildSessionEntry, o as chunkMarkdown, p as parseEmbedding, r as listSessionFilesForAgent, s as cosineSimilarity, t as requireNodeSqlite, u as isMemoryPath } from "./sqlite-ClMH0gmy.js";
...
async function fetchVoyageBatchStatus(params) {
	const baseUrl = normalizeBatchBaseUrl(params.client);
	const res = await fetch(`${baseUrl}/batches/${params.batchId}`, { headers: buildBatchHeaders(params.client, { json: true }) });
	if (!res.ok) {
		const text = await res.text();
...
async function readVoyageBatchError(params) {
	try {
		const baseUrl = normalizeBatchBaseUrl(params.client);
		const res = await fetch(`${baseUrl}/files/${params.errorFileId}/content`, { headers: buildBatchHeaders(params.client, { json: true }) });
		if (!res.ok) {
			const text = await res.text();
...
async function runVoyageEmbeddingBatches(params) {
	return await runEmbeddingBatchGroups({
		...
		runGroup: async ({ group, groupIndex, groups, byCustomId }) => {
			...
			const baseUrl = normalizeBatchBaseUrl(params.client);
			const contentRes = await fetch(`${baseUrl}/files/${completed.outputFileId}/content`, { headers: buildBatchHeaders(params.client, { json: true }) });
			if (!contentRes.ok) {
...
var MemoryManagerSyncOps = class {
	...
	async runSync(params) {
		...
		const needsFullReindex = params?.force || !meta || this.provider && meta.model !== this.provider.model || this.provider && meta.provider !== this.provider.id || meta.providerKey !== this.providerKey || meta.chunkTokens !== this.settings.chunking.tokens || meta.chunkOverlap !== this.settings.chunking.overlap || vectorReady && !meta?.vectorDims;
		try {
			if (needsFullReindex) {
...
function tokenize(text) {
	...
	for (const segment of segments) if (/[\u4e00-\u9fff]/.test(segment)) {
