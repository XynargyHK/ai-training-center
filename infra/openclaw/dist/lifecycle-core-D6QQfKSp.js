...
import { B as theme, R as colorize, U as getResolvedLoggerSettings, z as isRich } from "./utils-CFnnyoTP.js";
...
import { l as pickPrimaryLanIPv4 } from "./ws-CCvxoPJE.js";
import { t as isWSL } from "./wsl-B9lIxgu5.js";
...
import { a as checkTokenDrift, n as renderSystemdUnavailableHints } from "./systemd-hints-DMRp-4Mk.js";
...
async function runServiceRestart(params) {
	...
	if (params.checkTokenDrift) try {
		...
		const driftIssue = checkTokenDrift({
			serviceToken,
			configToken: loadConfig().gateway?.auth?.token || process.env.OPENCLAW_GATEWAY_TOKEN || process.env.CLAWDBOT_GATEWAY_TOKEN
		});
		if (driftIssue) {
