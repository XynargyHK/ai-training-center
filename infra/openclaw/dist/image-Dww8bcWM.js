...
import { R as shortenHomeInString, k as isRecord } from "./registry-BVeYESaO.js";
import { B as resolveImplicitProviders, L as normalizeProviders, R as resolveImplicitBedrockProvider, q as normalizeSecretInput, r as requireApiKey, t as getApiKeyForModel, y as resolveOpenClawAgentDir, z as resolveImplicitCopilotProvider } from "./model-auth-XLztKWJW.js";
import { n as discoverModels, t as discoverAuthStorage } from "./pi-model-discovery-DIA4gIzW.js";
import { N as sanitizeUserFacingText } from "./pi-embedded-helpers-Bn983kLW.js";
import { n as loadConfig } from "./config-BNjb0X52.js";
import { n as redactToolDetail } from "./redact-9hYpOXID.js";
...
function stripShellPreamble(command) {
	...
	for (let i = 0; i < 4; i += 1) {
		...
		].filter((candidate) => candidate.index >= 0).toSorted((a, b) => a.index - b.index)[0];
		const head = (first ? rest.slice(0, first.index) : rest).trim();
		if (!(head.startsWith("set ") || head.startsWith("export ") || head.startsWith("unset "))) break;
...
function summarizeExecCommand(command) {
	...
	if (!stage) return cleaned ? summarizeKnownExec(trimLeadingEnv(splitShellWords(cleaned))) : void 0;
...
function resolveExecDetail(args) {
	...
	if (!raw) return;
	const summary = summarizeExecCommand(unwrapShellWrapper(raw)) ?? summarizeExecCommand(raw) ?? "run command";
	const cwd = (typeof record.workdir === "string" ? record.workdir : typeof record.cwd === "string" ? record.cwd : void 0)?.trim();
