...
import { D as normalizeE164, R as truncateUtf16Safe, j as resolveUserPath, k as resolveConfigDir, ot as resolvePreferredOpenClawTmpDir, t as CHANNEL_IDS } from "./registry-CvUzfgyU.js";
import { i as resolveGatewayPort, t as STATE_DIR, u as resolveRequiredHomeDir } from "./paths-DJmOcr7Q.js";
import { $ as isPidAlive, Q as resolveProcessScopedMap } from "./model-selection-CB9IIVWU.js";
import { V as parseDurationMs, a as writeConfigFile, n as loadConfig, s as parseByteSize, t as createConfigIO } from "./config-XgvPhLbB.js";
import { f as resizeToJpeg, l as getImageMetadata, m as extensionForMime, p as detectMime } from "./ssrf--zCZt1NJ.js";
import { i as defaultRuntime, t as createSubsystemLogger } from "./subsystem-CPwOCKbN.js";
import { n as runExec } from "./exec-CBwIXprv.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-DqqvFxSv.js";
import { _ as DEFAULT_TOOLS_FILENAME, d as DEFAULT_AGENTS_FILENAME, f as DEFAULT_AGENT_WORKSPACE_DIR, g as DEFAULT_SOUL_FILENAME, h as DEFAULT_IDENTITY_FILENAME, l as resolveSessionAgentId, m as DEFAULT_HEARTBEAT_FILENAME, n as resolveAgentConfig, p as DEFAULT_BOOTSTRAP_FILENAME, v as DEFAULT_USER_FILENAME, y as ensureAgentWorkspace } from "./agent-scope-BmRbl8vE.js";
import { a as syncSkillsToWorkspace, l as resolveSandboxInputPath, m as safeEqualSecret, u as resolveSandboxPath } from "./skills-CFdAEWMp.js";
import { n as formatErrorMessage, t as extractErrorCode } from "./errors-7SO55DQM.js";
import { C as DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, D as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, E as DEFAULT_OPENCLAW_BROWSER_ENABLED, O as isLoopbackHost, S as DEFAULT_AI_SNAPSHOT_MAX_CHARS, T as DEFAULT_OPENCLAW_BROWSER_COLOR, a as resolveOpenClawUserDataDir, b as DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH, c as captureScreenshot, d as normalizeCdpWsUrl, f as snapshotAria, h as fetchOk, i as launchOpenClawChrome, l as createTargetViaCdp, m as fetchJson, n as isChromeCdpReady, o as stopOpenClawChrome, p as appendCdpPath, r as isChromeReachable, s as resolveBrowserExecutableForPlatform, v as ensureChromeExtensionRelayServer, w as DEFAULT_BROWSER_EVALUATE_ENABLED, x as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, y as stopChromeExtensionRelayServer } from "./chrome-CNaxD22L.js";
import { n as parseBooleanValue } from "./env-DNXtq8Zy.js";
import { d as getChannelDock, o as normalizeThinkLevel } from "./thinking-CtxhcKkY.js";
import { t as normalizeChatType } from "./chat-type-CE6cJOyt.js";
import { t as formatCliCommand } from "./command-format-Bc6VPcNk.js";
import { r as normalizeChannelId } from "./plugins-BBPPyP5O.js";
import { a as listDeliverableMessageChannels, o as normalizeMessageChannel } from "./message-channel-CzbBSsGC.js";
...
import fs, { createWriteStream, existsSync, realpathSync } from "node:fs";
	...
	if (process.platform === "win32") {
		...
		try {
			...
			for (let i = 0; i < 5; i++) try {
				...
				if (i === 4) console.warn(`[session-store] rename failed after 5 attempts: ${storePath}`);
			}
		} catch (err) {
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-DycKgxIG.js");
	} catch (err) {
...
function resolveBodyTargetId(body) {
	if (!body || typeof body !== "object" || Array.isArray(body)) return;
	return toStringOrEmpty(body.targetId) || void 0;
}
...
function registerBrowserAgentStorageRoutes(app, ctx) {
	...
	app.post("/cookies/clear", async (req, res) => {
		const profileCtx = resolveProfileContext(req, res, ctx);
		if (!profileCtx) return;
		const targetId = resolveBodyTargetId(readBody(req));
		try {
			const tab = await profileCtx.ensureTabAvailable(targetId);
...
function createProfileContext(opts, profile) {
	...
	const resetProfile = async () => {
		...
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-DycKgxIG.js")).closePlaywrightBrowserConnection();
		} catch {}
...
//#endregion
export { normalizeToolName as $, isTransientHttpError as A, countToolResults as At, resolveProfile as B, resolveChannelResetConfig as Bt, isContextOverflowError as C, deliveryContextFromSession as Ct, isRateLimitAssistantError as D, normalizeSessionDeliveryFields as Dt, isLikelyContextOverflowError as E, normalizeDeliveryContext as Et, resolveSandboxContext as F, normalizeInputProvenance as Ft, resolveBrowserControlAuth as G, resolveFreshSessionTotalTokens as Gt, resolvePathsWithinRoot as H, resolveSessionResetType as Ht, resolveSandboxRuntimeStatus as I, acquireSessionWriteLock as It, buildPluginToolGroups as J, deriveSessionMetaPatch as Jt, resolveSandboxConfigForAgent as K, canonicalizeMainSessionAlias as Kt, createBrowserRouteContext as L, resolveSessionLockMaxHoldFromTimeout as Lt, parseImageSizeError as M, INPUT_PROVENANCE_KIND_VALUES as Mt, sanitizeUserFacingText as N, applyInputProvenanceToUserMessage as Nt, isRawApiErrorPayload as O, archiveSessionTranscripts as Ot, ensureSandboxWorkspaceForSession as P, hasInterSessionUserProvenance as Pt, mergeAlsoAllowPolicy as Q, registerBrowserRoutes as R, resolveSessionKey as Rt, isCompactionFailureError as S, resolveCacheTtlMs as St, isFailoverErrorMessage as T, mergeDeliveryContext as Tt, getBridgeAuthForPort as U, resolveThreadFlag as Ut, DEFAULT_UPLOAD_DIR as V, resolveSessionResetPolicy as Vt, ensureBrowserControlAuth as W, DEFAULT_RESET_TRIGGERS as Wt, expandPolicyWithPluginGroups as X, collectExplicitAllowlist as Y, resolveGroupSessionKey as Yt, expandToolGroups as Z, formatRawAssistantErrorForUi as _, recordSessionMetaFromInbound as _t, isMessagingToolDuplicateNormalized as a, ensureSessionHeader as at, isBillingAssistantError as b, updateSessionStoreEntry as bt, extractToolCallsFromAssistant as c, sanitizeGoogleTurnOrdering as ct, isAntigravityClaude as d, saveMediaBuffer as dt, resolveToolProfilePolicy as et, isGoogleModelApi as f, extractDeliveryInfo as ft, formatBillingErrorMessage as g, readSessionUpdatedAt as gt, formatAssistantErrorText as h, loadSessionStore as ht, isMessagingToolDuplicate as i, buildBootstrapContextFiles as it, parseImageDimensionError as j, extractToolCallNames as jt, isTimeoutErrorMessage as k, capArrayByJsonBytes as kt, extractToolResultId as l, extractOriginalFilename as lt, classifyFailoverReason as m, resolveMirroredTranscriptText as mt, validateGeminiTurns as n, compileGlobPatterns as nt, normalizeTextForComparison as o, resolveBootstrapMaxChars as ot, BILLING_ERROR_USER_MESSAGE as p, appendAssistantMessageToSessionTranscript as pt, applyOwnerOnlyToolPolicy as q, resolveMainSessionKey as qt, pickFallbackThinkingLevel as r, matchesAnyGlobPattern as rt, sanitizeSessionMessagesImages as s, resolveBootstrapTotalMaxChars as st, validateAnthropicTurns as t, stripPluginOnlyAllowlist as tt, downgradeOpenAIReasoningBlocks as u, getMediaDir as ut, getApiErrorPayloadFingerprint as v, updateLastRoute as vt, isFailoverAssistantError as w, deliveryContextKey as wt, isCloudCodeAssistFormatError as x, isCacheEnabled as xt, isAuthAssistantError as y, updateSessionStore as yt, resolveBrowserConfig as z, evaluateSessionFreshness as zt };