import { Mt as resolveOpenClawAgentDir, Vt as DEFAULT_CONTEXT_TOKENS, l as normalizeProviderId } from "./model-selection-CqXyRThV.js";
...
function resolveAnthropicSonnet46ForwardCompatModel(provider, modelId, modelRegistry) {
	...
	const trimmedModelId = modelId.trim();
	const lower = trimmedModelId.toLowerCase();
	if (!(lower === ANTHROPIC_SONNET_46_MODEL_ID || lower === ANTHROPIC_SONNET_46_DOT_MODEL_ID || lower.startsWith(`${ANTHROPIC_SONNET_46_MODEL_ID}-`) || lower.startsWith(`${ANTHROPIC_SONNET_46_DOT_MODEL_ID}-`))) return;
	const templateIds = [];
	if (lower.startsWith(ANTHROPIC_SONNET_46_MODEL_ID)) templateIds.push(lower.replace(ANTHROPIC_SONNET_46_MODEL_ID, "claude-sonnet-4-5"));
	if (lower.startsWith(ANTHROPIC_SONNET_46_DOT_MODEL_ID)) templateIds.push(lower.replace(ANTHROPIC_SONNET_46_DOT_MODEL_ID, "claude-sonnet-4.5"));
...
//#endregion
export { buildModelAliasLines as a, resolveForwardCompatModel as i, resolveModel as n, ANTIGRAVITY_OPUS_46_FORWARD_COMPAT_CANDIDATES as r, buildInlineProviderModels as t };