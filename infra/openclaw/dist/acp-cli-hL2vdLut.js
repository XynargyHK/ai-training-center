...
import { t as DANGEROUS_ACP_TOOLS } from "./dangerous-tools-B7yH-Zod.js";
import { t as isMainModule } from "./is-main-BX87ziCN.js";
...
function registerAcpCli(program) {
	...
	acp.command("client").description("Run an interactive ACP client against the local ACP bridge").option("--cwd <dir>", "Working directory for the ACP session").option("--server <command>", "ACP server command (default: openclaw)").option("--server-args <args...>", "Extra arguments for the ACP server").option("--server-verbose", "Enable verbose logging on the ACP server", false).option("--verbose, -v", "Verbose client logging", false).action(async (opts, command) => {
