"""
Phone Bot v17 — PROVEN config from file-history snapshot @v17 (Apr 9 15:22 HKT)
- Transport: FastAPIWebsocketTransport + TwilioFrameSerializer (mulaw 8kHz)
- STT: Deepgram multi (auto-detect language) — HARDCODED, no per-lang switching
- LLM: Gemini 2.0 Flash
- TTS: Azure TTS with AutoTTSVoiceSwapper (swaps voice per detected language)
- File: phone_bot_v17.py (isolated prototype, do not modify)
"""
import asyncio
import os
import sys
...
class AutoTTSVoiceSwapper(FrameProcessor):
    """v17: Auto-swap Azure TTS voice based on detected language from STT.
    Reads TranscriptionFrame.language and updates tts._settings.voice live.
    Handles BOTH Deepgram codes (zh, yue, ja, ko, fr, ...) and Azure codes (zh-HK, ja-JP, ...).
    """
    VOICE_MAP = {
        # Deepgram-style short codes (Deepgram multi returns these)
        ...
        # Azure-style BCP-47 codes (in case provider returns these)
            ...
            if lang:
                ...
                if lang_str != self._current_lang:
                    ...
                    if new_voice != old_voice:
                        ...
                        logger.info(f"v17 TTS voice swap: {old_voice} → {new_voice} (lang: {lang_str})")
...
async def run_phone_bot(websocket_server_host, websocket_server_port, stream_sid, call_sid, from_number, to_number):
    ...
    # --- STT: Deepgram MULTI auto-detect (v17 PROVEN config) ---
    from pipecat.services.deepgram.stt import DeepgramSTTService
    ...
    logger.info("Phone STT: Deepgram multi (auto-detect, v17 — legacy run_phone_bot)")
...
追加銷售策略（確認預約之後先講）：
- 確認完預約之後，自然咁推薦加配服務。唔好硬銷，要好似朋友建議咁。
- 例如：「順便同你講，而家加配頸部護理有八折優惠，只需要多四百蚊，有冇興趣呀？」
- 如果客人話唔使，唔好再推。講：「冇問題嘅，聽日見！」
- 如果客人有興趣，確認加配服務同新價錢。
- 追加銷售項目：
  1. 頸部護理加配 — 原價五百，而家八折，四百蚊
  2. 精油升級 — 加一百五十蚊
  3. 眼部護理 — 加二百蚊
  4. 新客套餐 — 首次七折

重要：你打呢個電話係要確認預約。開場白已經講咗，而家等對方回應。確認完預約之後，自然咁推薦一個加配服務。""",
...
async def run_phone_bot_fastapi(websocket, stream_sid, call_sid, from_number, to_number, lang="en"):
    ...
    # --- STT: Deepgram MULTI auto-detect (v17 PROVEN config) ---
    # Hardcoded "multi" — no per-language switching. Deepgram detects language per utterance
    # and tags TranscriptionFrame.language so the voice swapper can react.
    from pipecat.services.deepgram.stt import DeepgramSTTService
    stt = DeepgramSTTService(
        api_key=os.getenv("DEEPGRAM_API_KEY"),
        language="multi",
    ...
    logger.info("Phone STT: Deepgram multi (auto-detect, v17)")
...
- Say goodbye naturally when the conversation ends.""",
    ...
    # --- v17: Auto TTS voice swapper (reads TranscriptionFrame.language) ---
    auto_tts_swapper = AutoTTSVoiceSwapper(tts)

    # --- Pipeline (v17: STT → swapper → user_aggregator → LLM → TTS) ---
    pipeline = Pipeline(
        [
            transport.input(),
            stt,
