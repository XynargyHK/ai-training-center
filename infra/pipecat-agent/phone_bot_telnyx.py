...
async def run_phone_bot(websocket_server_host, websocket_server_port, stream_sid, call_sid, from_number, to_number):
    ...
    # --- STT: Deepgram with specific language (faster than Azure) ---
    from pipecat.services.deepgram.stt import DeepgramSTTService
    deepgram_lang = {"yue": "zh-HK", "zh": "zh", "ja": "ja", "ko": "ko", "en": "en"}
    stt_lang = deepgram_lang.get(call_lang, "multi")
    stt = DeepgramSTTService(
        api_key=os.getenv("DEEPGRAM_API_KEY"),
        language=stt_lang,
        sample_rate=8000,
    )
    logger.info(f"Phone STT: Deepgram ({stt_lang})")
...
- Say goodbye naturally when the conversation ends.""",
    ...
    # --- Transport: Telnyx WebSocket (legacy run_phone_bot — not used by /dialtelnyx, kept for symmetry) ---
    serializer = TelnyxFrameSerializer(
        stream_id=stream_sid or "pending",
...
async def run_phone_bot_fastapi(websocket, stream_sid, call_sid, from_number, to_number, lang="en"):
    ...
    # --- STT: Deepgram with specific language (testing zh-HK on phone) ---
    from pipecat.services.deepgram.stt import DeepgramSTTService
    deepgram_lang = {"yue": "zh-HK", "zh": "zh", "ja": "ja", "ko": "ko", "en": "en"}
    stt_lang = deepgram_lang.get(call_lang, "multi")
    stt = DeepgramSTTService(
...
- Say goodbye naturally when the conversation ends.""",
    ...
    # --- Transport: FastAPI WebSocket via Telnyx Media Streams ---
    # Telnyx sends raw mulaw 8kHz frames; stream_id and call_control_id come from 'start' event.
    serializer = TelnyxFrameSerializer(
        stream_id=stream_sid or "pending",
        outbound_encoding="PCMU",
        inbound_encoding="PCMU",
