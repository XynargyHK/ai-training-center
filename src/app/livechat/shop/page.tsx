...
function LiveChatShopContent() {
  ...
  const [noBusinessUnit, setNoBusinessUnit] = useState(false)
  ...
  // Landing page data for header/announcement
  const [landingPage, setLandingPage] = useState<any>(null)

  useEffect(() => {
    if (!businessUnitParam) {
      setNoBusinessUnit(true)
      setLoading(false)
      return
    }
    setNoBusinessUnit(false)
    fetchProducts()
    loadAIData()
    fetchLandingPage()

    // Check if we should open cart (from landing page CTA)
  ...
  }, [businessUnitParam, searchParams])
  ...
  const fetchProducts = async () => {
    try {
      ...
      params.set('businessUnit', businessUnitParam) // Always pass business unit
  ...
  // Show message if no business unit specified
  if (noBusinessUnit) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8">
          ...
          <p className="text-gray-600 mb-6">
            Please access this shop through a valid business unit link.
          </p>
          <Link
            href="/"
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <ShopHeader
        ...
        businessUnitName={businessUnit?.name || 'Shop'}
        announcements={landingPage?.announcements || []}
        menuItems={landingPage?.menu_items || []}
        logoUrl={landingPage?.logo_url || ''}
        logoText={landingPage?.logo_text || ''}
        logoPosition={landingPage?.logo_position || 'center'}
      ...
      <div className="flex flex-1">
        {/* Left Sidebar */}
        <aside className="w-64 bg-white border-r border-gray-200 flex-shrink-0 sticky top-[104px] h-[calc(100vh-104px)] overflow-y-auto hidden md:block">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-6">
          {loading ? (
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">{selectedTypeName}</h2>
            ...
            <div className="space-y-12">
              ...
              {productTypes.filter(t => t.name !== 'Booster').map(type => {
                const typeProducts = filteredProducts.filter(p => p.product_type_id === type.id)
              ...
              {productTypes.find(t => t.name === 'Booster') && (
                <section>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h2 className="text-2xl font-bold text-gray-900">Boosters</h2>
                      <p className="text-gray-500 text-sm mt-1">Enhance your skincare routine with targeted boosters</p>
                    </div>
                    <button
                      onClick={() => setSelectedTypeId(productTypes.find(t => t.name === 'Booster')?.id || null)}
                      className="text-purple-600 hover:text-purple-700 text-sm font-medium flex items-center gap-1"
                    >
                      View All ({productTypes.find(t => t.name === 'Booster')?.count})
                      <ChevronRight className="w-4 h-4" />
                    </button>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                    {filteredProducts
                      .filter(p => p.product_types?.name === 'Booster')
                      .slice(0, 6)
                      .map(product => (
                        <div
                          key={product.id}
                        ...
                        >
                          ...
                          <div className="p-3">
                            ...
                            <div className="flex items-center justify-between mt-2">
                              <span className="text-sm font-bold text-gray-900">${getPrice(product).toFixed(2)}</span>
                              <button
...
export default function LiveChatShopPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
