...
export default function SplitBlockEditor({ block, onUpdate, onMediaLibraryOpen, businessUnitId }: SplitBlockEditorProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Image/Video Upload */}
      <div>
        <label className="block text-sm font-medium text-slate-300 mb-2">Image or Video</label>
        <div className="flex flex-col gap-3">
          ...
          {data.image_url ? (
            <div className="relative inline-block">
              ...
              {data.image_url.match(/\.(mp4|webm|ogg|mov)$/i) ? (
                <video
                  src={data.image_url}
                  controls
                  className="h-32 w-48 object-cover rounded-lg border border-slate-600"
                />
              ) : (
                <img
                  src={data.image_url}
            ...
            <div className="h-32 w-48 bg-slate-800 border-2 border-dashed border-slate-600 rounded-lg flex items-center justify-center">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={uploading}
              className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50 flex items-center gap-2"
            >
              {uploading ? (
                <>
          ...
          <input
            ...
            className="hidden"
          />
          <p className="text-xs text-slate-500">Supports: Images (JPG, PNG, GIF, WebP) and Videos (MP4, WebM, MOV)</p>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500 resize-y"
        />
      </div>

      {/* Optional CTA Button */}
      <div className="border-t border-slate-600 pt-4">
        <label className="block text-sm font-medium text-slate-300 mb-3">Call-to-Action Button (Optional)</label>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Button URL</label>
            <input
              type="text"
              value={data.cta_url || ''}
