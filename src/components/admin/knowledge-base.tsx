...
import {
  ...
  ChevronDown, ChevronUp, Zap, MessageSquare, Shield, ShoppingCart, Star,
...
const KnowledgeBase: React.FC<KnowledgeBaseProps> = ({ 
  ...
  const [setAsHomeOnCreate, setSetAsHomeOnCreate] = useState(false)
  const [globalAnnouncement, setGlobalAnnouncement] = useState<string[]>([])
  const [globalNavigation, setGlobalNavigation] = useState<any>({})
  const [globalFooter, setGlobalFooter] = useState<any>({})
  const [savingGlobal, setSavingGlobal] = useState(false)

  // Function to set current page as homepage for this locale
  const handleSetAsHome = async () => {
    if (!businessUnitId) return
    setIsSettingHome(true)
    try {
      const localeKey = `${selectedCountry}/${selectedLangCode}`
      const newConfig = { ...homepageConfig, [localeKey]: selectedSlug || '' }
      
      const response = await fetch(`/api/business-units/update-homepage`, {
        method: 'POST',
      ...
      if (result.success) {
        ...
        alert(`Success: "${selectedSlug || 'Home Page'}" is now the primary landing page for ${selectedCountry}/${selectedLangCode}`)
      } else {
        alert('Failed to update home page: ' + (result.error || 'Unknown error'))
      }
    } catch (err) {
      console.error('Set home error:', err)
      alert('Error saving home page configuration')
    } finally {
  ...
  const saveGlobalAssets = async () => {
    ...
    try {
      const response = await fetch('/api/business-units/update-global-assets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ businessUnitId, globalAnnouncement, globalNavigation, globalFooter })
      })
      const result = await response.json()
      if (!result.success) alert('Failed to save global settings: ' + (result.error || 'Unknown error'))
    } catch (err) {
      console.error('Save global assets error:', err)
    } finally {
  ...
  const loadLandingPage = async (country?: string, langCode?: string, slug?: string | null) => {
    ...
    try {
      ...
      // Update homepage config and global assets from business unit data
      if (data.businessUnit) {
        const config = data.businessUnit.homepage_config || {}
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await response.json()
      if (data.url) {
        setGlobalNavigation((prev: any) => ({ ...prev, logo_url: data.url }))
      }
    } catch (error: any) {
              ...
              <div>
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-1 group bg-gray-50 border border-gray-200 px-2 py-1">
                          <label className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter mr-1">Switch Page:</label>
                          <div className="relative flex items-center gap-1">
                            ...
                            >
                              {/* Only show 'Home Page' option if a home page actually exists for this specific locale */}
                              {(allPages || []).some(p => p.country === selectedCountry && p.language_code === selectedLangCode && p.slug === null) && (
                                <option value="home">🏠 Home Page</option>
                              )}
                              
                              {(allPages || [])
                                .filter(p => p.country === selectedCountry && p.language_code === selectedLangCode && p.slug !== null)
                                .map(p => {
                                  const localeKey = `${selectedCountry}/${selectedLangCode}`
                                  const isAssignedHome = homepageConfig[localeKey] === p.slug
                                  return (
                                    <option key={p.id} value={p.slug}>
                                      {isAssignedHome ? '🏠 ' : '📄 /'}{p.slug}
                                    </option>
                                  )
                                })
                              }
                            ...
                            <ChevronDown className="w-2.5 h-3 text-gray-400 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" />
                          </div>
                        </div>
                      </div>
                ...
                {landingPageLoading ? (
                  ...
                  <>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-left">
                            <h3 className="text-xs font-semibold text-amber-600">{t.announcementBanner} <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-1 py-0.5 rounded ml-1">🌐 Global</span></h3>
                            <p className="text-[10px] text-gray-500">Shared across all pages</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-gray-500">
                            {globalAnnouncement.length} items
                          </span>
                          <button
                            onClick={(e) => { e.stopPropagation(); saveGlobalAssets() }}
                            disabled={savingGlobal}
                            className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-none text-xs flex items-center gap-1 disabled:opacity-50"
                          >
                            {savingGlobal ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />}
                            Save
                          </button>
                      ...
                      {!collapsedSections.announcement && (
                        <div className="p-2 pt-1.5 space-y-2 border-t border-gray-200/30">
                          <div className="flex justify-between items-center">
                            <p className="text-xs text-gray-500">Add announcements that rotate every 5 seconds (global — saved to all pages)</p>
                            <div className="flex items-center gap-2">
                              <select
                                onChange={(e) => {
                                  if (e.target.value) {
                                    setGlobalAnnouncement([...globalAnnouncement, e.target.value])
                                    e.target.value = ''
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                const updated = [...globalAnnouncement, '']
                                    setGlobalAnnouncement(updated)
                                }}
                          ...
                          {(globalAnnouncement.length === 0) ? (
                            ...
                            <div className="space-y-1.5">
                              {globalAnnouncement.map((announcement: string, index: number) => (
                                <div key={index} className="flex gap-2 items-center">
                                  ...
                                  <button
                                    onClick={() => {
                                      const updated = globalAnnouncement.filter((_: string, i: number) => i !== index)
                                      setGlobalAnnouncement(updated)
                                    }}
                    ...
                    <div className="bg-gradient-to-r from-indigo-900/20 to-slate-800/50 rounded-none border border-indigo-500/30">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-left">
                            <h3 className="text-xs font-semibold text-indigo-600">{t.menuBar} <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-1 py-0.5 rounded ml-1">🌐 Global</span></h3>
                            <p className="text-[10px] text-gray-500">Shared across all pages</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-gray-500">
                            {(globalNavigation.menu_items || []).length} items
                          </span>
                          <button
                            onClick={(e) => { e.stopPropagation(); saveGlobalAssets() }}
                            disabled={savingGlobal}
                      ...
                      {!collapsedSections.menuBar && (
                        <div className="p-2 pt-1.5 space-y-2 border-t border-gray-200/30">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                              <div>
                                ...
                                <select
                                  value={globalNavigation.logo_position || 'left'}
                                  onChange={(e) => setGlobalNavigation({...globalNavigation, logo_position: e.target.value})}
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  value={globalNavigation.logo_text || ''}
                                  onChange={(e) => setGlobalNavigation({...globalNavigation, logo_text: e.target.value})}
                              ...
                              <div className="md:col-span-2">
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                  {globalNavigation.logo_url ? (
                                    <div className="relative">
                                      <img
                                        src={globalNavigation.logo_url}
                                        alt="Logo preview"
                                        className="h-10 w-auto max-w-[120px] object-contain bg-white rounded-none p-1"
                                      />
                                      <button
                                        onClick={() => setGlobalNavigation({...globalNavigation, logo_url: ''})}
                                  ...
                                  >
                                    {logoUploading ? (
                                      ...
                                      <>
                                        <Upload className="w-4 h-4" />
                                        {globalNavigation.logo_url ? 'Change' : 'Upload'}
                                      </>
                                    )}
                                  </button>
                                  <input
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {(globalNavigation.menu_items || []).map((item: { label: string; url: string; enabled: boolean }, index: number) => (
                                <div key={index} className="bg-gray-50 p-2 rounded-none">
                                  <div className="flex items-center gap-2 mb-2">
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const updated = [...(globalNavigation.menu_items || [])]
                                        updated[index] = { ...item, enabled: e.target.checked }
                                        setGlobalNavigation({...globalNavigation, menu_items: updated})
                                      }}
                                    ...
                                    <button
                                      onClick={() => {
                                        const updated = (globalNavigation.menu_items || []).filter((_: any, i: number) => i !== index)
                                        setGlobalNavigation({...globalNavigation, menu_items: updated})
                                      }}
                            ...
                            <button
                              onClick={() => {
                                const updated = [...(globalNavigation.menu_items || []), { label: '', url: '#', enabled: true }]
                                setGlobalNavigation({...globalNavigation, menu_items: updated})
                              }}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="bg-gray-50 p-3 rounded-none">
                                <label className="flex items-center gap-2 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={globalNavigation.show_search !== false}
                                    onChange={(e) => setGlobalNavigation({...globalNavigation, show_search: e.target.checked})}
                                    className="w-4 h-4 rounded-none border-gray-200 bg-gray-100 text-indigo-500 focus:ring-indigo-500"
                                  />
                              ...
                              <div className="bg-gray-50 p-3 rounded-none">
                                <label className="flex items-center gap-2 cursor-pointer mb-2">
                                  <input
                                    type="checkbox"
                                    checked={globalNavigation.show_account !== false}
                                    onChange={(e) => setGlobalNavigation({...globalNavigation, show_account: e.target.checked})}
                                    className="w-4 h-4 rounded-none border-gray-200 bg-gray-100 text-indigo-500 focus:ring-indigo-500"
                                  />
                                  <span className="text-xs text-gray-600">👤 My Account</span>
                                </label>
                                {globalNavigation.show_account !== false && (
                                  <input
                                    type="text"
                                    value={globalNavigation.account_url || '/account'}
                                    onChange={(e) => setGlobalNavigation({...globalNavigation, account_url: e.target.value})}
                                    placeholder="/account"
                                    className="w-full px-2 py-1 bg-white border border-gray-200 rounded-none text-gray-800 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                  />
                                )}
                              ...
                              <div className="bg-gray-50 p-3 rounded-none">
                                <label className="flex items-center gap-2 cursor-pointer mb-2">
                                  <input
                                    type="checkbox"
                                    checked={globalNavigation.show_cart !== false}
                                    onChange={(e) => setGlobalNavigation({...globalNavigation, show_cart: e.target.checked})}
                                    className="w-4 h-4 rounded-none border-gray-200 bg-gray-100 text-indigo-500 focus:ring-indigo-500"
                                  />
                                  <span className="text-xs text-gray-600">🛒 Shopping Cart</span>
                                </label>
                                {globalNavigation.show_cart !== false && (
                                  <input
                                    type="text"
                                    value={globalNavigation.cart_url || '/cart'}
                                    onChange={(e) => setGlobalNavigation({...globalNavigation, cart_url: e.target.value})}
                                    placeholder="/cart"
                                    className="w-full px-2 py-1 bg-white border border-gray-200 rounded-none text-gray-800 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                  />
                                )}
                    ...
                    {/* Page URL Slug (MIDDLE) */}
                    <div className="bg-gray-50 border border-gray-200 rounded-none p-2">
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-xs font-medium text-gray-600">Page URL Slug</label>
                        
                        <label className="flex items-center gap-1.5 cursor-pointer group">
                          <input 
                            type="checkbox"
                            checked={homepageConfig[`${selectedCountry}/${selectedLangCode}`] === (landingPageData.slug || '')}
                            onChange={handleSetAsHome}
                            disabled={isSettingHome}
                            className="w-3.5 h-3.5 rounded-none border-gray-300 text-violet-600 focus:ring-violet-500"
                          />
                          <span className="text-[10px] font-bold text-gray-500 uppercase tracking-tight group-hover:text-violet-600 transition-colors">
                            Set as Homepage
                          </span>
                          {isSettingHome && <Loader2 className="w-3 h-3 animate-spin text-violet-600" />}
                        </label>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <input
                          ...
                          className="flex-1 px-2 py-1 bg-white border border-gray-200 rounded-none text-gray-800 text-xs placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-violet-500 font-bold"
                        />
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-between mb-2 gap-2">
                        ...
                        {(() => {
                          ...
                          const updateFooter = (key: string, value: any) => {
                            setGlobalFooter((prev: any) => ({ ...prev, [key]: value }))
                          }
                      ...
                      <div className="mt-2 border border-gray-200 rounded-none overflow-hidden">
                        ...
                        >
                          <h3 className="text-xs font-medium text-gray-800 flex items-center gap-2">
                            📜 Footer & Policies <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-1 py-0.5 rounded ml-1">🌐 Global</span>
                          </h3>
                          <div className="flex items-center gap-2">
                            <button
                              onClick={(e) => { e.stopPropagation(); saveGlobalAssets() }}
          ...
          <div className="bg-white rounded-none w-full max-w-md p-4 border border-gray-200 shadow-2xl animate-in zoom-in duration-200">
            ...
            <div className="flex items-center gap-2 mt-6">
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-gray-500 hover:bg-gray-100 transition-colors uppercase tracking-wider border border-gray-200"
              >
                Cancel
              </button>
              
              <div className="flex-1 flex items-center gap-3 bg-violet-50 p-1 border border-violet-100">
                <button
                  onClick={async () => {
                    if (!newPageSlug) {
                    ...
                    try {
                      ...
                      const result = await response.json()
                      if (result.success) {
                        // If checkbox is checked, set as home page
                        if (setAsHomeOnCreate) {
                          const localeKey = `${selectedCountry}/${selectedLangCode}`
                          const newConfig = { ...homepageConfig, [localeKey]: newPageSlug }
                          await fetch(`/api/business-units/update-homepage`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ businessUnitId, homepageConfig: newConfig })
                          })
                          setHomepageConfig(newConfig)
                        }
                  ...
                  className="flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-700 text-white px-6 py-2 text-xs font-bold rounded-none shadow-md disabled:opacity-50 transition-all uppercase tracking-wider"
                >
                  {isProcessing ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3.5 h-3.5" />}
                  Create Page
                </button>

                <label className="flex items-center gap-2 cursor-pointer group px-2 border-l border-violet-200">
                  <input 
                    type="checkbox"
                    checked={setAsHomeOnCreate}
                    onChange={(e) => setSetAsHomeOnCreate(e.target.checked)}
                    className="w-4 h-4 rounded-none border-gray-300 text-violet-600 focus:ring-violet-500"
                  />
                  <span className="text-[10px] font-bold text-violet-700 uppercase tracking-tight group-hover:underline">
                    Set as Homepage
                  </span>
                </label>
              </div>
