...
interface TableBlockSSRProps {
  data: TableBlockData
  heading?: string
  anchorId?: string
}

export default function TableBlockSSR({ data, heading = '', anchorId }: TableBlockSSRProps) {
  const {
  ...
  if (!table_data || table_data.length === 0) return null
    ...
    >
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse" style={{ borderColor: border_color }}>
            <thead>
              <tr>
                {table_data[0]?.map((cell, colIndex) => (
                  <th
                    ...
                    style={{ borderColor: border_color, fontSize: table_font_size, color: table_text_color }}
                  >
                    {cell}
                  </th>
                ))}
